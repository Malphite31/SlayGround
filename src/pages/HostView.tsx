...
export function HostView() {
    ...
    // Countdown timer for performance - only runs when finished and countdown is showing
    useEffect(() => {
        if (status !== 'finished' || !showCountdown) return;
    ...
    }, [status, countdown, showCountdown]);

    // Fireworks effect when game finishes
    useEffect(() => {
                    ...
                    <>
                        ...
                        >
                            ...
                            <h1 className="text-8xl md:text-9xl font-heading font-black text-transparent bg-clip-text bg-gradient-to-br from-yellow-200 via-yellow-400 to-yellow-600 drop-shadow-[0_0_30px_rgba(234,179,8,0.5)] leading-none mb-4">
                                {winners.length > 1 ? 'WINNERS' : winners.length === 1 ? 'WINNER' : 'GAME COMPLETE'}
                            </h1>
                            <p className="text-2xl text-yellow-100/50 font-bold tracking-[0.4em] uppercase">
                                {winners.length > 0 ? 'The Slayground Champion' : 'Mission Accomplished'}
                            </p>
                        </motion.div>

                        {/* Show podium only if there are students */}
                        {top3.length > 0 ? (
                            ...
                            >
                                <p className="text-3xl text-slate-300 font-bold mb-4">No Players Joined</p>
                                <p className="text-xl text-slate-400">The game finished without any participants.</p>
                            </motion.div>
                        )}
                    </>
                )}
    ...
    // Waiting Screen with QR Code (before game starts or when paused)
    if ((status as string) !== 'playing' && (status as string) !== 'finished') {
